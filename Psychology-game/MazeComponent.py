...
class MazeComponent:
	...
	'''room_type is the type of room this component is (Hallways type). 
		...
		to turn (versus going on a straight trek). prize is a list 
		whose first item is the inventory type, the second is the amount
		and the third is if it has been distributed yet. prize_order 
		contains the prizes in order, when is_choice_room = True. options is the 
		options given by ReadFile. las_pos is simply the last visited 
		position before getting here.'''
	def __init__(self, room_type = None, colours = None, is_choice_room = False, 
		possible_prize = None, prize = None, prize_order = None, depth = 0, options = None,
		last_pos = None):
		self.adjacent = {}
		...
		if prize_order == None:
			...
			self.prize_order = prize
		
		self.depth = depth
		self.options = options
		
		if colours == None:
			self.colours = [[random.random()/2 for x in range(3)] for x in range(GLComponents.num_colours[room_type])]
		else:
