...
class Output2d:
	...
	def draw_inv_text(self):	#Box around the text here
		...
		for item_num in range(len(self.inventory.inventory)):	#Longest sentence
			...
			if cap:
				message = "{0}: {1:d}%".format(key, int(self.inventory.inventory[key]/cap * 100.0))
			else:
				message = "{0}: {1:d}".format(key, int(self.inventory.inventory[key]))
			...
			GLComponents.write_str(message, 0, text_top - item_num * 15, FONT, [1, 1, 1])
...
#				GLComponents.image_2d(((0, 0), (0, 100), (100, 100), (100, 0)), inv_types[0])
				GLComponents.image_2d(((87, 310), (87, 410), (150, 378), (150, 310)), inv_types[0])
			if inv_types[1] != None:
				GLComponents.image_2d(((width/2 - 50, 0), (width/2 - 50, 100), (width/2 + 50, 100), (width/2 + 50, 0)), inv_types[1])
			if inv_types[2] != None:
				GLComponents.image_2d(((width - 150, 310), (width - 150, 378), (width - 87, 410), (width - 87, 310)), inv_types[2])
				
	#For showing the user information on a black background
	#A possible padding is allowed. Here it is given as a percent (where 1 is 100%)
	...
	#not used, see code below that just changes background image
	def draw_selected(self):
		message = 'You selected %s.' % self.maze.position.prize[0]
		self.draw_centred(message)	

	#change background image to show user what direction they selected
	def draw_selected_dirn(self, selected_dirn): 
	#not set up for backwards. Selected dirn is 'left' 'centre' or 'right'
		verts = ((0, 0), (0, SCREEN_SIZE[1]), (SCREEN_SIZE[0], SCREEN_SIZE[1]), (SCREEN_SIZE[1], 0)) #entire screen
		GLComponents.image_2d(verts, SELECTED_DIRN_IMAGES[selected_dirn])
